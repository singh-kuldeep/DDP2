/*! \file dt.h
    \brief This header file conditions the function TimeStep() which calculate the local time step for each cell at every iteration.
    \author Kuldeep Singh
    \date 2016
    \sa grid()
    \bug Currently not using this, because  grid() is not calculating ds value properly. 
    ...
    \param [in] delta_s ds value of the cell for which TimeStep is to be calculated
...
double TimeStep(int i,int j, int k, vector<vector<vector<double> > > delta_s, vector<vector<vector<vector<double> > > > ConservedVariables){
	...
	double VelocityMagnitude = sqrt(pow((ConservedVariables[i][j][k][1]/ConservedVariables[i][j][k][0]),2) +
	pow((ConservedVariables[i][j][k][2]/ConservedVariables[i][j][k][0]),2) +
	pow((ConservedVariables[i][j][k][3]/ConservedVariables[i][j][k][0]),2) );

	double Pressure = (SpecificHeatRatio-1)*(ConservedVariables[i][j][k][4] - 0.5*ConservedVariables[i][j][k][0]*VelocityMagnitude*VelocityMagnitude);
	double VelocitySound = sqrt(SpecificHeatRatio*Pressure/ConservedVariables[i][j][k][0]);
	double TimeStep = (CFL*delta_s[i][j][k])/(VelocitySound+VelocityMagnitude);
	// cout <<  "TimeStep   i " << i << " j " << j << " k " << k << " TimeStep = " << TimeStep << endl;
	return TimeStep;
} 